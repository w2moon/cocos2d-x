...
static JSBool js_cocos2dx_CCImagePicker_useCamera(JSContext *cx, uint32_t argc, jsval *vp)
{
     if (argc >= 1) {
		...
		if( argc >= 3 ) {
            JS_ValueToBoolean(cx, argv[3], &canedit );
        }

        
		picker->useCamera(tmpCobj,callfuncO_selector(JSCCImagePickerWrapper::responseCallbackO),canedit);
...
static JSBool js_cocos2dx_CCImagePicker_usePhotoLibrary(JSContext *cx, uint32_t argc, jsval *vp)
{
     if (argc >= 1) {
		...
		picker->usePhotoLibrary(tmpCobj,callfuncO_selector(JSCCImagePickerWrapper::responseCallbackO),canedit);
...
void register_all_cocos2dx_extension_manual(JSContext* cx, JSObject* global)
{
	...
	JS_DefineFunction(cx, jsb_CCImagePicker_prototype, "useCamera", js_cocos2dx_CCImagePicker_useCamera, 3, JSPROP_READONLY | JSPROP_PERMANENT);
    JS_DefineFunction(cx, jsb_CCImagePicker_prototype, "usePhotoLibrary", js_cocos2dx_CCImagePicker_usePhotoLibrary, 3, JSPROP_READONLY | JSPROP_PERMANENT);
